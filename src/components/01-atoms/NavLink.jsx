...
export default function AppLink({ href, children, className = "", underline = true, underlineColor = "bg-primary-text", highlighted = false, bold = false, extrabold = false, asListItem = false, ...props }) {
  ...
  const underlineClass = underline ? `absolute -bottom-1 left-0 w-full h-0.5 ${underlineColor} origin-left transform transition-transform duration-500 ${isActive ? "scale-x-100" : "scale-x-0"} group-hover:scale-x-100 group-focus:scale-x-100` : "";
